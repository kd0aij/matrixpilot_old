...
void handleMessage(mavlink_message_t* msg)
{
    ...
    case 0x51:  // 0x51(decimal 81) temporary work around. Numbering not syncing up correctly for 0.76 beta QGroundControol,  
				// where MAVLINK_MSG_ID_REQUEST_DATA_STREAM is hardocded in as decimal 66.
    {
        // decode
...
void mavlink_output_40hz( void )
{
	...
	if (mavlink_frequency_send( 4, counter_40hz + spread_transmission_load)) 
	{
		...
		mavlink_msg_sys_status_send(MAVLINK_COMM_0, mode, MAV_NAV_WAYPOINT, MAV_STATE_ACTIVE, 
			( uint16_t) 500 , 	// (uint16_t) (udb_cpu_load()) * 10, 
