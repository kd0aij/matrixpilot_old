...
void update_coords()
{
	...
	*/
	serial_output("$A,%li,%li,%i,%i,%i,%i,%i,\r\n",
		...
		(int)svs,
		IMUlocationz._.W1,		
		(int)(ground_speed_3DIMU/100),	// speed in m/s
		(int)(air_speed_3DIMU/100),	// speed in m/s
		angle,			
		gps_fix_type
	);
}
...
static void update_wp()
{
	struct relative2D curHeading ;
	curHeading.x = -rmat[1] ;
	curHeading.y = rmat[4] ;
	int earth_yaw = rect_to_polar(&curHeading) ;// 0-255 (0=East,  ccw)
	int dir_to_goal = desired_dir - earth_yaw ;
