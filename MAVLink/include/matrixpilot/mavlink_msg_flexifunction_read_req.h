...
#define MAVLINK_MESSAGE_INFO_FLEXIFUNCTION_READ_REQ { \
	...
	4, \
	{  { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_flexifunction_read_req_t, target_system) }, \
         { "target_component", NULL, MAVLINK_TYPE_UINT8_T, 0, 1, offsetof(mavlink_flexifunction_read_req_t, target_component) }, \
         { "read_req_type", NULL, MAVLINK_TYPE_INT16_T, 0, 2, offsetof(mavlink_flexifunction_read_req_t, read_req_type) }, \
         { "data_index", NULL, MAVLINK_TYPE_INT16_T, 0, 4, offsetof(mavlink_flexifunction_read_req_t, data_index) }, \
         } \
}
						       ...
						       uint8_t target_system, uint8_t target_component, int16_t read_req_type, int16_t data_index)
{
	...
	_mav_put_int16_t(buf, 2, read_req_type);
	_mav_put_int16_t(buf, 4, data_index);
...
static inline void mavlink_msg_flexifunction_read_req_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, int16_t read_req_type, int16_t data_index)
{
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FLEXIFUNCTION_READ_REQ, buf, 6);
#else
	mavlink_flexifunction_read_req_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FLEXIFUNCTION_READ_REQ, (const char *)&packet, 6);
#endif
}
